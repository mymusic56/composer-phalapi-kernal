...
```
    ...
    protected function isServiceWhitelist() {
        ...
        foreach ($serviceWhitelist as $item) {
            ...
            // 短路返回
            if ($this->equalOrIngore($namespace, $cfgArr[0]) && $this->equalOrIngore($api, $cfgArr[1])) {
                $this->__apiIsServiceWhitelist = TRUE;
                break;
            }
            if(count($cfgArr)==3&&$this->equalOrIngore($namespace, $cfgArr[0]) &&$this->equalOrIngore($api, $cfgArr[1]) && $this->equalOrIngore($action, $cfgArr[2])){
                $this->__apiIsServiceWhitelist = TRUE;
                break;
            }
        }
