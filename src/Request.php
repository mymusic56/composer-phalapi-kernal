...
class Request {
    ...
    /**
     * @var array $data 主数据源，接口原始参数 不包含cookie
     */
    protected $data_nocookie = array();
...
```    
    ...
    public function __construct($data = NULL) {
        ...
        $this->data_nocookie= $this->genData_nocookie($data);
        // 备用数据源
        $this->get      = $_GET;
        $this->post     = $_POST;
     ...
     * @return array
     */
    protected function genData_nocookie($data) {
        if (!isset($data) || !is_array($data)) {
...
```     
     ...
     * @return array
     */
    public function getAllNocookie() {
        return $this->data_nocookie;
    }
